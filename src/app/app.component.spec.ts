...
describe("AppComponent", () => {
  ...
  it("should contain innings", async () => {
    await createComponent();
    expect(screen.queryByText("Inning")).toBeInTheDocument();
  });
  ...
  describe.each([
    ...
    it("should increase value", async () => {
      const { add, display } = await createComponent({
        home: 0,
        away: 0,
        inning: 0,
      });
    ...
    it("should decrease value", async () => {
      const { remove, display } = await createComponent({
        home: 1,
        away: 1,
        inning: 1,
      });
  ...
  describe("outs", () => {
      ...
      it.each([
        {
          ...
          expectedOuts: 0,
          expect: (
            firstOut: () => Element | null,
            secondOut: () => Element | null,
          ) => {
        ...
        "should show $expectedOuts out for $outs out in the inning",
        async ({ outs, expectedOuts, expect }) => {
    ...
    describe("when removing outs", () => {
      it.each([
        ...
        "should show $expectedOuts outs after removing an out from $outs outs in the inning",
        async ({ outs, expectedOuts, expect }) => {
          const { remove, firstOut, secondOut } = await createComponent({
            outsInInning: outs,
          });
          await remove(/outs/i);
          expect(firstOut, secondOut);
  ...
  const createComponent = async (
    partialState: Partial<ScoreBoardState> = {},
  ) => {
